...
class Ur5Reach:
    ...
    def __init__(self, robot, models: Models, camera=None, vis=False) -> None:
        ...
        self.goal_range_low = np.array([-0.3 / 2, -0.3 / 2, 0])  # goal_xy_range = 0.3,
        self.goal_range_high = np.array([0.3 / 2, 0.3 / 2, 0.3])
        self.action_space_low = -1.0
        self.action_space_high = 1.0
        self.action_shape = 7
        self.task_input_shape = 3
    ...
    def get_achieved_goal(self) -> np.ndarray:
        ee_pos = p.getLinkState(self.robot.id, self.robot.eef_id)[0]  # end effector of the arm without a gripper, a tuple type
        gripper_tip_pos = np.array(ee_pos) - [0, 0, 0.14]  # gripper is approximately 0.14 in length
