...
class PyBullet:
    ...
    def create_box(
        ...
        if texture is not None:
            texture_path = os.path.join(my_panda_gym.assets.get_data_path(), texture)
    ...
    def create_slippery_table(
        ...
        self.create_box(
            ...
            position=np.array([x_offset, 0.0, -height / 2]),
            specular_color=np.zeros(3),
            rgba_color=np.array([97.0/255.0, 226.0/255.0, 1.0, 1]),
            lateral_friction=lateral_friction,
            spinning_friction=spinning_friction,
        )
